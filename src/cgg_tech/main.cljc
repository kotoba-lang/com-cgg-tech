(ns cgg_tech.main
  "Kotodama WASM entrypoint for the Cgg_tech clean-room actor (L4) — Clojure port.
  ...
  facts (`cgg_tech.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cgg_tech")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"cgg_tech.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cgg_tech-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cgg_tech-compat/ready)
